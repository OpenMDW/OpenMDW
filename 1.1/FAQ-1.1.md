# OpenMDW-1.1 License Agreement FAQ

## 1. What is the purpose of the OpenMDW‑1.1 license?

The OpenMDW License Agreement v1.1 ("OpenMDW‑1.1") is a permissive open‑source license crafted specifically for machine‑learning models and their associated artifacts ("Model Materials"). Its goal is to provide a single, clear framework that providers of Model Materials can apply to grant rights to downstream users under all relevant intellectual‑property regimes, including copyright, patent, database and trade secrets.

OpenMDW-1.1 is a minor release, with one change as compared to [OpenMDW-1.0](https://github.com/OpenMDW/OpenMDW/tree/main/1.0): the change from "patent" to "patent or copyright" in the fifth paragraph. See question 6 below for more details.

## 2. How are "Model Materials" defined?

Under OpenMDW‑1.1, **Model Materials** include:

1. **Machine‑learning models** (architecture and parameters); and
2. **All related artifacts** (including associated data, documentation and software) that are provided under OpenMDW-1.1.

Item 1 is intended to include model components such as architecture definitions and learned parameters. Item 2 is intended to include related artifacts such as training or evaluation datasets, code (preprocessing, inference, training scripts), documentation, metadata and supporting tools; in each case for item 2, when the model provider chooses to make them available under OpenMDW-1.1.

This broad definition enables components that are critical to reproducing or extending a model to be licensed consistently. While OpenMDW-1.1 does not mandate completeness (in the sense of requiring all related components to be provided), it does provide terms that ensure the ability to use in an open and free manner any and all components that are distributed under OpenMDW-1.1.

## 3. What rights does OpenMDW‑1.1 grant?

Subject to compliance with its terms, OpenMDW‑1.1 grants **unrestricted, royalty‑free permission** to "deal in the Model Materials without restriction, including under all copyright, patent, database, and trade secret rights included or embodied therein."
...
Applicable laws might impose their own requirements or restrictions, which of course no license is able to override. But OpenMDW-1.1 does not impose any such restrictions of its own.
...
## 6. How does the patent and copyright litigation clause work?

OpenMDW‑1.1 includes a **patent and copyright litigation‑termination** provision: if you initiate or maintain a lawsuit alleging that the Model Materials (directly or indirectly) infringe a patent or copyright--except in defensive response to a suit first filed against you--**all rights granted to you under OpenMDW‑1.1 terminate immediately**.

Although the scope is broader, this provision is intended to be comparable to "defensive termination" provisions relating to patents in other licenses such as Apache-2.0. Defensive termination provisions cannot ensure the absence of patent aggression, such as from "non-practicing entities" or other patent holders that don't make use of the OpenMDW-1.1 materials themselves. However, this provision aims to discourage aggressive patent or copyright assertions from among the users of the Model Materials that could hinder open collaboration.

## 7. Are outputs generated by the model subject to the license's attribution requirements?

**No.** For any outputs you generate by using the Model Materials, **no restrictions or obligations** are imposed by OpenMDW‑1.1 for using, modifying or sharing those outputs.
...
## 9. How do I apply OpenMDW‑1.1 to my project?

1. **Copy the full text** of the OpenMDW‑1.1 license into a file named `LICENSE` at the root of your repository.
2. **Ensure all Model Materials** (models, code, data, etc.) that you want to distribute under OpenMDW-1.1 are contained in that repository.
3. Include **copyright notices** in each file or artifact that you want downstream users to preserve.
4. **Include any additional documentation** (e.g. `README.md`) describing your project, and referencing the OpenMDW-1.1 license.
...
## 10. Is OpenMDW‑1.1 compatible with other open‑source licenses?

**In many cases, yes, we believe so.** "License compatibility" is a complex and fact-specific topic. But in many cases (especially those involving other "permissive" open source licenses) OpenMDW-1.1 is likely to be compatible, because it is possible to comply with both of them at the same time.

Because it is permissive and imposes minimal conditions, you can generally **combine** Model Materials licensed under OpenMDW‑1.1 with code under MIT, Apache-2.0, the BSD family of licenses, and similarly permissive licenses. Care should be taken if integrating with copyleft licenses (e.g. GPL-3.0), as their terms may impose additional requirements on the combined work.

Keep in mind that you generally may not take third-party materials that you received under another license, and change its license to OpenMDW-1.1 without the licensor's permission. If you redistribute third-party materials under other licenses, you should be sure to comply with those other licenses' terms in your distribution.

## 11. Can a distribution that uses OpenMDW-1.1 also include materials that are under other licenses?
...
## 12. Is "OpenMDW-1.1" a valid SPDX license ID?

Not at present (as of May 2026). The OpenMDW License Agreement, version 1.1, has just been announced and has not yet been submitted to SPDX for consideration for the [SPDX License List](https://spdx.org/licenses). We will update this FAQ if and when it is added to the SPDX License List.
...
OpenMDW-1.1 does not impose a particular citation format. Model contributors may include a CITATION.cff file or citation instructions in the README file.

## 14. How should OpenMDW-1.1 be referenced in model cards?

Model cards are short documents accompanying trained machine learning models that provide benchmarked evaluation in a variety of conditions. To reference OpenMDW-1.1, it is recommended to set the License field in the model card metadata and link to the full license text of this repository, with reference to the 1.1/ folder. You can also include a short explanation in the usage or limitations section of the model card if it exists.
